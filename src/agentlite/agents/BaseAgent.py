...
class BaseAgent(ABCAgent):
    ...
    def __add_st_memory__(self, short_term_memory: AgentSTMemory = None):
        ...
        if short_term_memory:
            ...
            self.short_term_memory = DynamicErrorSTMemory(agent_id=self.id)
    ...
    def load_ehr(self, task):
        observation = self.ehr_manager.load_ehr_for_sample(task.ehr_db, task.timestamp)
        action = AgentAct(name="LoadEHR", params={"file": f"{task.ehr_db}"})
    ...
    def llm_layer(self, prompt: str) -> str:
        ...
        outputs = self.llm.run(prompt)
        self.logger.get_thinking(outputs["reasoning"])       
