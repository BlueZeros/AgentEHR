...
class MemoryExtractor:
    ...
    def __init__(self, llm, temperature, max_items):
        """
        Args:
            llm: LLM model
        ...
        self.prompt_gen = ReasoningBankPromptGen(max_items=max_items)
    ...
    def extract_memories(
        ...
        if isinstance(trajectory, list):
            ...
            if len(step_strs) > 1:
                last_step = step_strs.pop()  # Keep last step aside
                
                while count_tokens(step_strs + [last_step]) > MAX_TOKENS and len(step_strs) > 0:
                    del step_strs[-1]  # Remove the latest history step (从后往前截断)
                
                step_strs.append(last_step)  # Put last step back
            
            trajectory_str = "".join(step_strs)
        else:
            trajectory_str = str(trajectory)

        # Select appropriate prompt using prompt generator
        if success:
            prompt = self.prompt_gen.extract_success_prompt(query, trajectory_str, final_state, model_output)
        else:
            prompt = self.prompt_gen.extract_failure_prompt(query, trajectory_str, final_state, model_output)
